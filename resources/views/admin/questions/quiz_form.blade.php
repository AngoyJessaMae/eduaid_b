...
<div class="max-w-3xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-gray-900 mb-8">
        {{ isset($question) ? '✏️ Edit Question' : 'Add Question to ' . $quiz->title }}
    </h1>

    <div class="bg-white rounded-lg shadow p-8">
        <form method="POST" action="{{ isset($question) ? route('admin.questions.quiz.update', [$quiz, $question]) : route('admin.questions.quiz.store', $quiz) }}">
            @csrf
            @if(isset($question))
                @method('PUT')
            @endif
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.questions.quiz.index', $quiz) }}" class="flex-1 bg-gray-200 text-gray-900 font-bold py-3 rounded-lg hover:bg-gray-300 transition text-center">
                    Cancel
                </a>
            </div>
        </form>
